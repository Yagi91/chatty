import { forwardRef, ReactNode, use, useEffect, useRef } from "react";
import { TopicType } from "./types";

function categorize(topics: Array<TopicType> | null): {
  today: Array<TopicType>;
  last7Days: Array<TopicType>;
  rest: Array<TopicType>;
} {
  const A_DAY = 60 * 60 * 24 * 1000;
  const today: Array<TopicType> = [];
  const last7Days: Array<TopicType> = [];
  const rest: Array<TopicType> = [];
  if (topics === null) {
    return { today: [], last7Days: [], rest: [] };
  }
  for (let i = 0; i < topics.length; i++) {
    let topic = topics[i];
    let date = new Date(topic.created_at).valueOf();
    if (Date.now() < A_DAY + date) {
      today.push(topic);
    } else if (Date.now() < A_DAY * 7 + date) {
      last7Days.push(topic);
    } else {
      rest.push(topic);
    }
  }
...
function TopicItem({
  ...
  id: string;
  title: string;
  onClickCallback: (id: string) => Promise<void>;
}) {
  return (
    <a
      onClick={() => onClickCallback(id)}
      className="block cursor-pointer ease-in-out duration-200 hover:bg-zinc-700 px-2 py-1 rounded-md"
    >
      <p>{title}</p>
    </a>
  );
...
>(({ toggleSidebar, topics, loadThread, topicsLoading }, ref) => {
  let categorizedTopics = categorize(topics);
  return (
    <div
      className="max-md:w-80 w-0 ease-in-out md:transition-[width] max-md:transition-transform z-20 md:z-0 max-md:fixed max-md:inset-y-0 max-md:-left-[320px] !duration-300 "
      ref={ref}
    >
      <nav
      ...
      >
        ...
        <div className="space-y-4">
          <a className="gap-2 flex items-center text-sm">
            <span className="">
              <svg
                width="41"
                height="41"
        ...
        {topicsLoading ? (
          <div className="w-full">
            <div className="animate-pulse">
              <div className="bg-zinc-700 h-10 grow w-full mb-4 rounded"></div>
              <div className="bg-zinc-700 h-10 grow w-full mb-4 rounded"></div>
              <div className="bg-zinc-700 h-10 grow w-full mb-4 rounded"></div>
              <div className="bg-zinc-700 h-10 grow w-full mb-4 rounded"></div>
              <div className="bg-zinc-700 h-10 grow w-full mb-4 rounded"></div>
          ...
          <div>
            <p className="pt-4 pb-2 font-bold text-sm">Today</p>
            <div className="space-y-2">
              {categorizedTopics.today.map((topic) => (
                <TopicItem
                  id={topic.id}
                  onClickCallback={loadThread}
                  key={topic.id}
                  title={topic.title}
                />
              ))}
            ...
            {/* Last 7 Days */}
            <p className="pt-4 pb-2 font-bold text-sm">Previous 7 Days</p>
            <div className="space-y-2">
              {categorizedTopics.last7Days.map((topic) => (
                <TopicItem
                  onClickCallback={loadThread}
                  id={topic.id}
                  key={topic.id}
            ...
            {/* A long time ago */}
            <p className="pt-4 pb-2 font-bold text-sm">A long time ago</p>
            <div>
              {categorizedTopics.rest.map((topic) => (
                <TopicItem
                  onClickCallback={loadThread}
                  id={topic.id}
                  key={topic.id}
